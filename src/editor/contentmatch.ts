...
export class ContentMatch {
    ...
    get defaultType() {
        for (let i = 0; i < this.next.length; i += 2) {
            ...
            if (!(type.isText || type.hasRequiredProps())) return type
        }
    }
    ...
    fillBefore(after, toEnd = false, startIndex = 0) {
        ...
        function search(match, types) {
            ...
            for (let i = 0; i < match.next.length; i += 2) {
                ...
                if (!(type.isText || type.hasRequiredProps()) && seen.indexOf(next) == -1) {
                    seen.push(next)
    ...
    computeWrapping(target) {
        ...
        while (active.length) {
            ...
            for (let i = 0; i < match.next.length; i += 2) {
                ...
                if (!type.isLeaf && !type.hasRequiredProps() && !(type.name in seen) && (!current.type || match.next[i + 1].validEnd)) {
...
function checkForDeadEnds(match, stream) {
    for (let i = 0, work = [match]; i < work.length; i++) {
        ...
        for (let j = 0; j < state.next.length; j += 2) {
            ...
            if (dead && !(node.isText || node.hasRequiredProps())) dead = false
